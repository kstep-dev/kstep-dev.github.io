 ...
 * @returns {string[]}
 */
export function qemu_args(smp, mem, snapshot) {
    const ret = wasm.qemu_args(smp, mem, snapshot);
 ...
 * @returns {any}
 */
export function shm_decode(region) {
    const ptr0 = passArray8ToWasm0(region, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.shm_decode(ptr0, len0);
    if (ret[2]) {
 ...
 * @returns {number}
 */
export function shm_size() {
    const ret = wasm.shm_size();
    return ret >>> 0;
}
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('kstep_core_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
