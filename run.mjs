...
// status -> stderr, results -> --out (default results/<build>-<driver>/).
// --driver defaults to the build name minus _buggy/_fixed; --smp/--mem to reproduce.py's
...
const kernel = args.build ?? 'sync_wakeup_buggy';
...
try {  // defaults from site/data.json (build.sh), i.e. reproduce.py's values for the bug
  const b = JSON.parse(fs.readFileSync(path.join(W, 'site', 'data.json'))).bugs.find(b => b.name === defaults.driver);
